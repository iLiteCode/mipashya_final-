...
def user_profile_edit(request):
    ...
    if request.method == 'POST':
        ...
        if 'profile_image' in request.FILES:
            ...
            print("Profile image uploaded:", user.profile_image)
        if 'aadhar_image' in request.FILES:
            user.aadhar_image = request.FILES['aadhar_image']
            print("Aadhar image uploaded:", user.aadhar_image)
        if 'pancard_image' in request.FILES:
            user.pancard_image = request.FILES['pancard_image']
            print("Pancard image uploaded:", user.pancard_image)
        
        user.save()
        messages.success(request, "Profile updated successfully.")
        return redirect('user:user_profile')
    
    return render(request, 'user_login/user_profile_edit.html', {'user': user})# Staff Profile View
@login_required(login_url='/')
